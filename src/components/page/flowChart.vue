        ...
        <el-container class="content-el-container">
          <el-header height="42px" class="flex breadcrumb-head">
            <el-breadcrumb separator-class="el-icon-arrow-right">
              ...
              <el-breadcrumb-item :to="{ path: '/' }" @click.native="breadClick(index)"
                                  v-for="(item, index) in breadList" :key="index">{{ item.label }}
              </el-breadcrumb-item>
            </el-breadcrumb>
          </el-header>
...
<!--                    </div>-->
  ...
  <link rel="stylesheet" href="./static/css/nodeSetting.css">
          ...
          <el-container v-if="pageType !== 'editor'">
            <mappingSetting v-if="pageType === 'parameterMap'" :mappingData="mappingData"
                            @mappingLinksUpdate="mappingLinksUpdate"/>
          </el-container>
        </el-container>
      </el-main>
    </el-container>
...
export default {
  ...
  watch: {
    ...
    pageType(val) {
      switch (val) {
        case 'processParameterIn':
          this.paramsTableData = this.currentNodeData.dataSetting.paramsSetting.in || [];
          break
        case 'processParameterOut':
          this.paramsTableData = this.currentNodeData.dataSetting.paramsSetting.out || [];
          break
        case 'serviceParameterIn':
          this.paramsTableData = this.flowChartJson.serviceParamsSetting.in || [];
          break
        case 'serviceParameterOut':
          this.paramsTableData = this.flowChartJson.serviceParamsSetting.out || [];
          break
        default:
          break
      }
